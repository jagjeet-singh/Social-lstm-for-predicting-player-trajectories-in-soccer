...
def plot_trajectories(true_trajs, pred_trajs, nodesPresent, obs_length, name, plot_directory, withBackground=False):
    ...
    fig, ax = plt.subplots()
    ax.imshow(im, extent = [0,105,0,68])
    for j in traj_data:
        #c=[0.5,0.5,0.5]
    #for j in [1,22]:    
        if j < 11:
        ...
        #print str(min(true_x)) + ' : ' + str(max(true_x)) + '   ' +  str(min(true_y)) + ' : ' + str(max(true_y))
        #print str(min(pred_x)) + ' : ' + str(max(pred_x)) + '   ' +  str(min(pred_y)) + ' : ' + str(max(pred_y))
        #pdb.set_trace()
        #plt.plot(true_x, true_y, color=c, linestyle='solid', marker='o')
        #plt.plot(pred_x, pred_y, color=c, linestyle='dashed', marker='x')


        ax.plot(true_x, true_y, color=c, linestyle='solid', label = 'True trajectory')
        ax.plot(pred_x, pred_y, color=c, linestyle='dashed', linewidth=1, label = 'Predicted trajectory')
    ...
    if not withBackground:
        ...
        plt.xlim(0,1) 
    plt.show()
    #plt.savefig('result'+str(np.random.randint(100)), dpi=300)
    
    #if withBackground:
     #   plt.savefig('asdasascasx')
    #else:
...
def main():
    ...
    f = open(save_directory+'/results_30.pkl', 'rb')
