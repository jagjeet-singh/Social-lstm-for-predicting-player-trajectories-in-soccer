...
def getGridMask(frame, dimensions, neighborhood_size, grid_size):
    ...
    dimensions : This will be a list [width, height] # 720x576
    ...
    mnp = frame.shape[0] # 22
    width, height = dimensions[0], dimensions[1] #720, 576

    frame_mask = np.zeros((mnp, mnp, grid_size**2)) #22,22,16
...
def getSequenceGridMask(sequence, dimensions, neighborhood_size, grid_size):
    ...
    '''
    sl = len(sequence) # 20 in our case
    sequence_mask = []
    ...
    for i in range(sl):
        ...
        if use_cuda:
            ...
            sequence_mask.append(Variable(torch.from_numpy(getGridMask(sequence[i], dimensions, neighborhood_size, grid_size)).float()))
        #sequence_mask.append(Variable(torch.from_numpy(getGridMask(sequence[i], dimensions, neighborhood_size, grid_size)).float()).cuda())
