...
class DataLoader():
    ...
    def __init__(self, batch_size=50, seq_length=5, datasets=[0, 1, 2, 3, 4], forcePreProcess=False, infer=False):
        ...
        self.data_dirs = ['./data/mixed46_1', './data/mixed46_2', './data/mixed46_3',
                          './data/mixed46_4', './data/mixed46_5']

        #self.data_dirs = ['./data/eth/univ', './data/eth/hotel',
         #                 './data/ucy/zara/zara01', './data/ucy/zara/zara02',
          #                './data/ucy/univ', './data/soccer/coord_short']
    ...
    def frame_preprocess(self, data_dirs, data_file):
        ...
        for directory in data_dirs:
            ...
            data = np.genfromtxt(file_path, delimiter=',') # 4x46
            #pdb.set_trace()
            # Frame IDs of the frames in the current dataset
            frameList = np.unique(data[0, :]).tolist() #[0,1]
            numFrames = len(frameList) # [2]
            ...
            for ind, frame in enumerate(frameList):
                ...
                pedsInFrame = data[:, data[0, :] == frame] # 4x23 each time

                # Extract peds list
                pedsList = pedsInFrame[1, :].tolist() # [0,1,...,22]

                # Add number of peds in the current frame to the stored data
                numPeds_data[dataset_index].append(len(pedsList)) # 23

                # Initialize the row of the numpy array
                pedsWithPos = [] # list of length 23
        ...
        # all_frame_data: data_index x time_frames x numPeds x 2
        # frameList_data: list of frameIDs
        # numPeds_data: list of pedIDs and coordinates
        f = open(data_file, "wb")
    ...
    def load_preprocessed(self, data_file):
        ...
        self.valid_data = self.raw_data[3] # valied_frame_data
        counter = 0
        valid_counter = 0
        #pdb.set_trace()
        # For each dataset
        ...
        #for dataset in range(len(self.data)):
            ...
            print('Training data from dataset', dataset, ':', len(all_frame_data)) # len = 1
            print('Validation data from dataset', dataset, ':', len(valid_frame_data))
            # Increment the counter with the number of sequences in the current dataset
            counter += int(len(all_frame_data) / (self.seq_length)) # total_time_frames / sequence_length
            valid_counter += int(len(valid_frame_data) / (self.seq_length))
        #pdb.set_trace()
        # Calculate the number of batches
        self.num_batches = int(counter/self.batch_size) # num_sequences / batch_size
