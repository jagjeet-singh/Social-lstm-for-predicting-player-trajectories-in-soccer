...
def train(args):
    ...
    # Logging files
    log_file_curve = open(os.path.join(log_directory, 'log_curve_20.txt'), 'w')
    log_file = open(os.path.join(log_directory, 'val_20.txt'), 'w')
    ...
    with open(os.path.join(save_directory, 'config_20.pkl'), 'wb') as f:
        pickle.dump(args, f)

    # Path to store the checkpoint file
    def checkpoint_path(x):
        return os.path.join(save_directory, 'social_lstm_model_20_'+str(x)+'.tar')
